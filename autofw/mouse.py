...
class HumanMouse:
    ...
    def _generate_zigzag_points(
        self, start_x: float, start_y: float, end_x: float, end_y: float, num_nodes: int, variance: float
    ...
    def _compute_spline_trajectory(self, control_points: list[tuple[float, float]]) -> list[tuple[float, float]]:
        ...
        if len(control_points) < 4:
            ...
            return list(zip(np.interp(t, np.linspace(0, 1, len(x)), x), np.interp(t, np.linspace(0, 1, len(y)), y)))
        try:
            k = min(3, len(x) - 1)
